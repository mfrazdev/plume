...
// Adicionada a propriedade "linker" para Cross-Compile não usar o 'cc' errado
const targets = [
    ...
    { id: 'linux-arm64',   rustTarget: 'aarch64-unknown-linux-gnu', ext: 'a', prefix: 'lib', dotnetRid: 'linux-arm64', linker: 'aarch64-linux-gnu-gcc' },
    { id: 'alpine-amd64',  rustTarget: 'x86_64-unknown-linux-musl', ext: 'a', prefix: 'lib', dotnetRid: 'linux-musl-x64', linker: 'musl-gcc' },
...
for (const t of targetsToBuild) {
    ...
    try {
        if (buildRust) {
            ...
            // Configura o Linker Correto para Linux ARM e Alpine
            let buildEnv = { ...process.env };
            if (t.linker) {
                const envVarName = `CARGO_TARGET_${t.rustTarget.toUpperCase().replace(/-/g, '_')}_LINKER`;
                buildEnv[envVarName] = t.linker;
                console.log(`   [INFO] Forçando linker customizado: ${envVarName}=${t.linker}`);
            }

            const buildCmd = `cargo build --release --target ${t.rustTarget}`;
            execSync(buildCmd, { cwd: rustProjectDir, stdio: 'inherit', env: buildEnv });
        ...
        // FIX: Se der erro num Target, mata tudo na hora!
        console.error(`${RED}   [!] Abortando todos os processos subsequentes devido a este erro!${NC}`);
        process.exit(exitCode);
    }
    console.log("");
}
