        ...
        public static void Register(RouteGroupBuilder group)
        {
            group.MapPost("/servers/action", async (ActionPayload body, HttpContext context) =>
            {
                ...
                if (currentStatus.Equals("installing", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    return Results.Json(new { error = "server is installing" }, statusCode: 400);
                }

                var serverDb = DatabaseManager.Db.GetServer(body.ServerId);
                if (serverDb == null)
                    return Results.Json(new { error = "Server not found in database" }, statusCode: 404);
                ...
                if (!isStopOrKill && body.Disk != 0L)
                {
                    ...
                    if (diskUsageMB > body.Disk)
                    {
                        ...
                        return Results.Json(new { error = "Disk usage exceeds the limit" }, statusCode: 400);
                    }
                }
                ...
                switch (body.Action.ToLower())
                {
                    ...
                    case "install":
                        ...
                        {
                            if (!currentStatus.Equals("stopped", StringComparison.OrdinalIgnoreCase))
                                return Results.Json(new { error = "server not stopped" }, statusCode: 400);
                    ...
                    default:
                        return Results.Json(new { error = "Invalid action" }, statusCode: 400);
                }
